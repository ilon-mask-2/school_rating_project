...
def admin_view_student_account(root, student_id):
    from client.admin_view_accounts import admin_view_accounts
    from client.admin_edit_student_account import admin_edit_student_account
    ...
    tk.Label(container, text="👤 " + get_translation("student", module="shared_translation"),
    ...
    def populate_data(student):
        # === Фото ===
        if student.get("photo"):
            try:
                image_bytes = base64.b64decode(student["photo"])
        ...
        display("login", student['login'])
        display("email", student.get('email', '—'))
        display("phone", student.get('phone', '—'))

        # === Группы ===
        if student.get("groups"):
            tk.Label(main_frame, text=get_translation("groups", module="admin_view_student_account_translation"), font=("Roboto", 10, "bold"), bg="#FFFFFF").pack()
            for group in student["groups"]:
                tk.Label(main_frame, text=f"• {group}", bg="#FFFFFF").pack(anchor="w")

        # === Учителя ===
        if student.get("teachers"):
            tk.Label(main_frame, text=get_translation("teachers", module="admin_view_student_account_translation"), font=("Roboto", 10, "bold"), bg="#FFFFFF").pack(pady=(10, 0))
            for teacher in student["teachers"]:
                tk.Label(main_frame, text=f"• {teacher}", bg="#FFFFFF").pack(anchor="w")
    ...
    def load_student_data():
        try:
            ...
            root.after(0, lambda: populate_data(student))
        except Exception as e:
            root.after(0, lambda: messagebox.showerror(get_translation("error_title", module="errors_translation"),
                                                       f"{get_translation('error_load_student', module='admin_view_student_account_translation')}\n{e}"))

    threading.Thread(target=load_student_data).start()
    ...
    def edit_student():
        admin_edit_student_account(root, student_id)

    def delete_student():
        if not messagebox.askyesno(get_translation("confirm", module="shared_translation"),
                                   get_translation("confirm_delete_student", module="admin_view_student_account_translation")):
            return

        def task():
            try:
                requests.delete(f"{SERVER_URL}/admin/students/{student_id}")
                root.after(0, lambda: (
                    messagebox.showinfo(get_translation("deleted", module="shared_translation"),
                                        get_translation("student_deleted", module="admin_view_student_account_translation")),
                    go_back()
                ))
            except Exception as e:
                root.after(0, lambda: messagebox.showerror(get_translation("error_title", module="errors_translation"),
                                                           f"{get_translation('error_delete_student', module='admin_view_student_account_translation')}\n{e}"))
    ...
    tk.Button(button_frame, text=get_translation("edit", module="shared_translation"), command=edit_student, width=15,
              font=("Roboto", 10), bg="#BBDEFB", relief="flat").grid(row=0, column=1, padx=10)

    tk.Button(button_frame, text=get_translation("delete", module="shared_translation"), command=delete_student, width=15,
              font=("Roboto", 10), bg="#FFCDD2", relief="flat").grid(row=0, column=2, padx=10)

    add_language_switcher(root, lambda: admin_view_student_account(root, student_id))
