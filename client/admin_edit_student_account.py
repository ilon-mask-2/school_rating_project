...
def admin_edit_student_account(root, student_id):
    ...
    tk.Label(container, text="✏️ " + get_translation("edit", module="shared_translation") + " — " + get_translation("student", module="shared_translation"),
    ...
    fields = [
        ...
        (get_translation("class", module="shared_translation"), "class"),
    ...
    def populate_form(data):
        ...
        for idx, (label, key) in enumerate(fields):
            tk.Label(form_frame, text=label, bg="#FFFFFF", font=("Roboto", 10, "bold")).grid(row=idx, column=0, sticky="e", padx=5, pady=3)
            ...
            for key, var in field_vars.items():
                ...
                if key == "password":
                    if val:
                        ...
                        data["password"] = student[0].get("password", "")
                else:
                    data[key] = val or student[0].get(key, "")
            data["photo"] = photo_data[0] or student[0].get("photo")
            try:
                response = requests.put(f"{SERVER_URL}/admin/students/{student_id}", json=data)
                response.raise_for_status()
                root.after(0, lambda: (
                    messagebox.showinfo(get_translation("success_title", module="shared_translation"),
                                        get_translation("student_updated", module="admin_edit_student_account_translation")),
                    admin_view_student_account(root, student_id)
                ))
            except Exception as e:
                root.after(0, lambda: messagebox.showerror(get_translation("error_title", module="errors_translation"),
                                                           f"{get_translation('error_update_student', module='admin_edit_student_account_translation')}\n{e}"))
    ...
    def load_student():
        try:
            ...
            root.after(0, lambda: populate_form(student_data))
        except Exception as e:
            root.after(0, lambda: messagebox.showerror(get_translation("error_title", module="errors_translation"),
                                                       f"{get_translation('error_load_student', module='admin_view_student_account_translation')}\n{e}"))

    add_language_switcher(root, lambda: admin_edit_student_account(root, student_id))
